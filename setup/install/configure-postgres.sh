#!/usr/bin/env bash
# Post-install wiring for the postgresql package. Three steps the
# package itself doesn't handle:
#
...
# Real DB creation (`mix ecto.create`, etc.) stays per-project.
set -euo pipefail

if ! command -v initdb >/dev/null 2>&1; then
    echo "postgresql not installed; skipping" >&2
    exit 0
fi
...
# that. Bail loudly if it never accepts connections — better than
# hanging forever in the role-creation step below.
ready=0
...
# Create a $USER superuser if missing. `psql -tAc` returns "1" when the
# role exists, empty otherwise. Quoted role name guards against any
# odd characters in the username.

#!/usr/bin/env bash
# Build and install ghostty pinned to its latest stable git tag. Avoids the
# regression risk of tracking master via the AUR's ghostty-git package
# (which bit us once with broken background rendering after a master commit).
#
# Re-running this script automatically pulls a newer stable release if
# upstream has tagged one — no manual version bump required. To freeze on
# a specific version, replace the dynamic `LATEST_TAG` lookup below with
...
if [ -z "$LATEST_TAG" ]; then
  echo "install-ghostty: could not determine latest stable tag" >&2
  exit 1
fi

# Skip if installed version is already pinned to the latest tag.
# The PKGBUILD's pkgver() runs `git describe --long`, which for a checkout
# at an exact tag produces version "1.3.1.r0.g<sha>" — that's our marker.
...
# Pin AUR PKGBUILD's source to the chosen tag instead of master HEAD. The
# AUR uses a variable-driven URL ("git+$url.git"), so we replace the whole
# source= line and verify the change actually landed before building.
if ! grep -q 'source=("git+\$url.git")' PKGBUILD; then
  echo "install-ghostty: PKGBUILD's source line shape changed; sed needs updating" >&2
  exit 1
fi
